...
│       ├── gallery.json             # Manifest of gallery images for the gallery page
│       └── thumbs/                  # Generated thumbnails for faster gallery load
├── members/
...
### 2. Gallery Image Processing
The gallery is generated by `generate_gallery.sh` and uses ImageMagick/GraphicsMagick when available.

What it does:
- Scans `images/gallery/` for supported image files.
- Builds `images/gallery/gallery.json` as the gallery manifest.
- Generates optimized thumbnails in `images/gallery/thumbs/`.
- Resizes images to fit within `600x600` pixels.
- Uses `-quality 75` for JPEG compression.
- Strips metadata from both thumbnails and the original gallery images to reduce file size and remove identifying EXIF data.
- If ImageMagick/GraphicsMagick is missing, it skips thumbnail generation safely.

### 3. Updating Social Links & Handles
